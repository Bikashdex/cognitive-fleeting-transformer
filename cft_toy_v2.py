# cft_toy_v2.py — fixes: slot-identity init, destination-keyed gate, developmental curriculum
...
# ---------- SECTION 2: MODEL (two surgical fixes) ----------
class BDHCQ(nn.Module):
    ...
    def write(self, Pr, M, g):
        ...
        Upd = Aw.transpose(1, 2) @ self.wo(Pr)          # FIX 2a: aggregate per destination slot
        G   = torch.sigmoid(self.wg(Upd) + self.wg2(Md)) # FIX 2b: gate keyed to destination slot
        return Md + G * Upd
...
class CFTToy(nn.Module):
    def __init__(self):
        ...
        self.bdh   = BDHCQ()
        self.M0    = nn.Parameter(torch.randn(N_SLOTS, D)*.1)  # FIX 1: distinct slot identities
    ...
    def forward(self, chunks, types, ablate=False):
        ...
        for c, t in zip(chunks[:-1], types[:-1]):
            x = self.local(self.tok(c) + self.typ(t)); pools.append(x.mean(1))
            M = self.bdh.write(self.bdh.parts(x), M, g)
        Pq = self.bdh.parts(self.local(self.tok(chunks[-1]) + self.typ(types[-1])))
...
# ---------- SECTION 3: DEVELOPMENTAL CURRICULUM (FIX 3) ----------
...
for phase, (L, steps) in enumerate([(1, 2500), (2, 2500), (3, 1500)], 1):   # Infant -> Student
...
print(f"memory ABLATED (M zeroed), len 3: {acc(3, True):.2%}   <- NOW it must collapse to ~50%")