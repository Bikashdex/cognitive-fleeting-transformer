# cft_compare_v2.py — Added progress prints so you know it's working!
...
for phase, (L, steps) in enumerate(PHASES, 1):
    print(f"\n--- Starting Phase {phase} (chain length {L}, {steps} steps) ---")
    for step in range(steps):
        ...
        # Train Baseline
        i, t, y_b = batch_flat(128, L)
        lb = F.cross_entropy(base(i, t), y_b)
        ob.zero_grad(); lb.backward(); ob.step()
        
        # TALKATIVE UPDATE: Print every 500 steps so you know it's alive!
        if step % 500 == 0:
            print(f"  step {step:4d}/{steps} | CFT loss: {lc.item():.3f} | Baseline loss: {lb.item():.3f}")
